...
- COMPLIANCE_GATE_ENABLED — when `true`: (1) getSendReadiness additionally requires a recorded GDPR lawful basis + accepted outreach terms (and, for Canada-targeting workspaces, a CASL consent basis) before a workspace can launch outreach; and (2) the send pipeline (sendCampaignBatch and sendFollowupTask, in apps/worker) enforces per-RECIPIENT consent — any workspace with `lawfulBasis: 'consent'` or `targetsCanada: true` skips (fail-closed, audited as `consent.enforcement.skipped`) any contact lacking an on-file ConsentRecord, rather than relying on the workspace-level "at least one record exists" check alone. Default `false` (ships dormant) — turn on only after the legal copy (sub-processor list, T&Cs, DPA, LIA prompts) is signed off. The compliance posture is recorded via Settings → Compliance regardless of this flag.
