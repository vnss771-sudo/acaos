...
**Status: closed as of 2026-09-14**, implemented by a 5-seat council working in parallel isolated worktrees and merged sequentially onto `claude/run-comparison-oz9ccj` with a full independent re-verification after every merge (typecheck across all 5 packages, lint, full unit suite — 1649 tests — DB-tier suite against real Postgres — 329 tests — Redis-tier suite against real BullMQ/Redis, web suite, and the complete `npm run verify` governance + build pipeline). Two merges produced real conflicts (shared files touched by multiple seats) that were resolved and re-verified by hand rather than auto-merged blind; one real cross-cutting gap (the offline Prisma client stub missing a newly-referenced `Prisma.JsonValue`) was caught by `check:offline-stub` and fixed. Several seats' self-reported "N pre-existing test failures" turned out to be stale/incomplete `node_modules` or a stray ambient Redis process in that seat's own worktree, not real issues — each was independently reproduced-or-refuted in a clean environment before being trusted.

- **Observability at scale**: fixed real multi-replica metric-aggregation bugs (single static Prometheus scrape target; several alerts/dashboard panels double-counting Redis/DB-shared gauges across replicas instead of `max()`-ing them), added `ops/monitoring/recording_rules.yml`; added real OpenTelemetry distributed tracing (`packages/backend-core/src/lib/tracing.ts`) propagating a W3C `traceparent` from every API-side enqueue through to the worker's job processing, verified with a real cross-process trace-ID match — off by default (`OTEL_EXPORTER_OTLP_ENDPOINT`/`OTEL_CONSOLE_EXPORTER`), a genuine no-op otherwise.
- **DB & cache scaling**: Prisma `connection_limit` is now actually pinned (a sane computed default, configurable via `DB_POOL_SIZE`, never overriding an operator-set value) instead of only warned about; concrete PgBouncer sizing formula documented; `ingestCache`'s API-key/plan revocation now propagates across pods via Redis pub/sub (fail-open on a Redis outage) instead of only clearing the handling pod's own in-memory cache.
- **Queue & worker scaling**: queue-depth-adaptive worker concurrency (opt-in, hysteresis-damped, proven safe against real BullMQ — scaling down never abandons an in-flight job) and an automated DLQ auto-retry sweep for transient-looking failures beyond BullMQ's own exhausted attempts, both live-tested against real Redis.
- **Release confidence**: `tests/operational-chaos-safety-gates.test.ts` rewritten from source-text grepping to real behavior (verified each test actually catches a broken guard, not just a deleted one); `check-critical-test-coverage.mjs` now enforces real line/branch coverage floors (not just "a test file exists") across 20 safety-critical modules, closing a real gap it found along the way (`suppressions.ts` had no direct unit test); the existing load-test script widened to multi-tenant concurrent traffic plus a worker/queue-path phase (enqueue → poll → completion latency); a platform-agnostic canary bake/auto-rollback gate (`scripts/rollout-gate.mjs`, reusing the same burn-rate multiplier as the existing SLO alert) plus an auto-triggered post-deploy smoke job, both real code paths but unverified against an actual deploy target (none exists in this repo yet).
- **Architecture cleanup**: `sendCampaignBatch` decomposed from ~576 lines into 8 named, independently unit-tested steps; `Settings.tsx`/`Leads.tsx` split into per-section components (`components/settings/`, `components/leads/`) with the parent views now thin orchestrators — verified byte-for-byte behaviorally identical via existing tests plus live browser screenshots; all 23 `apps/api/src/lib/` re-export shims deleted and their ~126 call sites repointed at `@acaos/backend-core` directly, with the former ratchet script now a hard guarantee (zero shims, not just no *new* ones).

**Known unverified-pending-real-infra, by design, not oversight**: canary/rollout behavior against an actual deploy target; OTel span export against a real collector backend; Prometheus recording/alert rules against `promtool`/live multi-replica traffic (no `promtool` binary or multi-replica deployment exists in this sandbox). Each is real, reviewed code — the honesty note is about what running here could and couldn't prove.

## 7. Phase 3 — Unicorn polish (continuous)

**Status: closed as of 2026-09-15**, implemented by a 5-seat council working in parallel isolated worktrees and merged sequentially onto `claude/run-comparison-oz9ccj` with a full independent re-verification after every merge (typecheck across all 5 packages, lint, full unit suite — 1692 tests — DB-tier suite against an isolated real Postgres — 346 tests — web suite — 233 tests — and the complete `npm run verify` governance + build pipeline). One merge produced a real conflict (both seat 4 and seat 5 registered a new route module in the same workspace-router file) resolved by hand rather than auto-merged blind. Before writing any agent briefs, reconnaissance confirmed two of June's Phase-3 sub-items (the `ScoringOutcome` reply-signal wiring and the constant-`size` scoring dimension) were already done in Phase 1 — not redone here.

- **Learning loop that earns its name**: the hardcoded `MIN_OUTCOMES = 10` cold-start gate is now env-configurable (`LEARNING_LOOP_MIN_OUTCOMES`) with its statistical reasoning documented in code (bounds the baseline win rate's standard error each signal's lift multiplier pivots against); added exponential-decay recency weighting (`LEARNING_LOOP_RECENCY_HALF_LIFE_DAYS`, default 180d) to the weight-retuning math, verified to reduce identically to the old unweighted computation when no `recordedAt` is supplied and unable to bypass the raw sample-count floor; the eval harness's pass/fail gate (built in Phase 2) now also computes and persists a real score/lift trend across runs, committed back to the repo by CI, closing the "expose model lift" gap.
- **Explainable scoring surfaced to users**: Leads already had a real narrative explanation (`LeadBrief.tsx`, Phase 2); Prospects did not — only four raw numeric tiles. Fixed, and along the way found and fixed a live bug where `scoreBreakdown.contribution` was always `null` (read a nonexistent `Signal.weight` DB column instead of the real per-signal intent math). New `ProspectBrief.tsx` derives its plain-language "why this score" narrative entirely from real fields — dominant signal, ICP fit reasons, signal recency, source reliability — verified against real seeded data through a live browser session, not fabricated.
- **Deliverability program**: the fully-built warmup ramp (`warmup.ts`, Phase-1-or-earlier) was dead code — `warmupStartedAt` was read everywhere but set nowhere. Now auto-starts on a workspace's first successful SMTP save (once; later edits never reset an established sender's ramp) plus an explicit manual restart control; `DeliverabilitySection.tsx` now surfaces both warmup ramp status (day X of N, today's cap) and sender reputation (bounce/complaint rate, healthy/blocked verdict) sourced from the existing `evaluateSenderReputation`; confirmed "per-domain" isn't a real concept in this codebase (one SMTP config per workspace) so workspace-level Grafana panels were added for the metrics the worker already emitted but never had dashboard panels for. The daily-send-cap enforcement and the bounce/complaint→suppression feedback loop were both already correctly wired — confirmed, not rebuilt.
- **SOC2 track**: migrated several genuinely security-critical events (MFA activation, invite acceptance, workspace member add/remove, API-key rotate/revoke) from fire-and-forget `recordAudit` to durable `recordCriticalAudit`, including a real gap found along the way — a second, UI-unused but live `ingest.ts` API-key rotate/revoke path had zero audit trail at all. Verified `docs/KEY_ROTATION.md` against the actual `encrypt.ts` code and fixed one real mismatch (the doc only documented one of the two live ingest-key rotation routes). Built a genuine access-review mechanism (workspace-scoped and platform-wide, CSV-exportable, with a Settings UI section) — confirmed nothing like this existed before; added `User.lastLoginAt`, wired at the three real login-completion points, kept distinct from the pre-existing `lastReauthAt`.
- **In-product DPA/CASL consent capture**: audited the already-fairly-mature compliance route/panel against four candidate gaps and confirmed three real: (a) no real DPA document was ever surfaced in-product beyond the sub-processor list — added a versioned, acknowledgeable in-app DPA view; (b) `lawfulBasis`/`targetsCanada` were pure workspace metadata with no per-recipient enforcement — added a fail-closed per-recipient `ConsentRecord` check in the send pipeline (both campaign batch and follow-up sends), kept behind the existing dormant `COMPLIANCE_GATE_ENABLED` flag to match the codebase's "ships dormant until legal sign-off" pattern; (c) `ConsentRecord`s could previously only be created via manual admin action — wired automatic creation from CSV lead import when a recognized consent-basis/date column is present. CASL parity (d) was addressed by the same per-recipient gate rather than forcing an artificial GDPR/CASL equivalence the two regimes don't share.

**Known unverified-pending-real-infra, by design, not oversight**: a live Grafana/Prometheus instance actually rendering the new per-workspace deliverability panels; real deploy/infra behavior for anything above. One pre-existing, low-probability test-isolation flake (`OpsShifts.test.tsx`, from Phase 2, untouched by any Phase 3 seat) was independently observed by two different seats and confirmed here as environmental — not a regression — via repeated clean full-suite runs.
