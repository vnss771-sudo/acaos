...
### B2 — [P1, ops] Observability is wired in-cluster but not externally; Sentry aggregation is optional
**[CORRECTED — was overstated as P0 "errors silently swallowed"]** `errorReporting.ts` loads `@sentry/node` via a dynamic `await import(...)`; since it isn't vendored, `captureError()` is a no-op for *Sentry aggregation*. **But errors are NOT swallowed** — every `captureError` call site already logs the error first via the structured logger (`http.ts` `log.error('unhandled error', …)`, the `server.ts`/`worker.ts` uncaught handlers via `logLifecycleEvent(…'crash'…)`). So errors reach logs/your log aggregator today; Sentry is *optional* error aggregation an operator enables by installing `@sentry/node` + setting `SENTRY_DSN` (the dynamic import wires it up). It is deliberately NOT vendored — it drags a heavy, recurringly-vulnerable OpenTelemetry tree through dependency-review for no correctness gain. **Fix (S):** documented as opt-in; no code change needed for not-swallowing.
