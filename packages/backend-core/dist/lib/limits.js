...
const AI_ACTIONS = ['AI_RESEARCH', 'AI_OUTREACH', 'AI_REPLY'];
const DISCOVERY_ACTION = 'DISCOVERY';
const PLAN_LIMITS = {
    free: { aiCallsPerMonth: 15, maxLeads: 500, discoveriesPerMonth: 25 },
    starter: { aiCallsPerMonth: 300, maxLeads: 10000, discoveriesPerMonth: 500 },
...
async function getWorkspacePlan(workspaceId, client = prisma) {
    const ws = await client.workspace.findUnique({
        where: { id: workspaceId },
        select: { plan: true, subscriptionStatus: true }
    });
    // Treat lapsed subscriptions as free
    if (ws?.subscriptionStatus && ws.subscriptionStatus !== 'active')
        return 'free';
    return resolvePlan(ws?.plan);
}
async function getMonthlyAiCount(workspaceId) {
...
export async function checkAndIncrementAiUsage(workspaceId, action) {
    ...
    await prisma.$transaction(async (tx) => {
        ...
        if (isFinite(aiCallsPerMonth)) {
            ...
            if (used >= aiCallsPerMonth) {
                throw new ApiError(429, `Monthly AI limit reached (${aiCallsPerMonth} calls/month on ${plan} plan). ` +
...
export async function checkAndIncrementDiscoveryUsage(workspaceId) {
    const plan = await getWorkspacePlan(workspaceId);
    ...
    await prisma.$transaction(async (tx) => {
        ...
        if (isFinite(discoveriesPerMonth)) {
            ...
            if (used >= discoveriesPerMonth) {
                throw new ApiError(429, `Monthly discovery limit reached (${discoveriesPerMonth} runs/month on ${plan} plan). Upgrade to run more.`);
            }
        }
        await tx.usageRecord.upsert({
            where: { workspaceId_month_action: { workspaceId, month, action: DISCOVERY_ACTION } },
...
export async function checkLeadLimit(workspaceId) {
    const plan = await getWorkspacePlan(workspaceId);
    const { maxLeads } = PLAN_LIMITS[plan];
    if (!isFinite(maxLeads))
        return;
    const count = await prisma.lead.count({ where: { workspaceId } });
    if (count >= maxLeads) {
        throw new ApiError(429, `Lead limit reached (${maxLeads} leads on ${plan} plan). Upgrade to add more leads.`);
    }
}
/**
 ...
 */
export async function reserveLeadCapacity(tx, workspaceId, requested) {
    if (requested <= 0)
        return 0;
    await tx.$executeRaw `SELECT pg_advisory_xact_lock(hashtext(${workspaceId}))`;
...
export async function getMonthlyUsage(workspaceId) {
    ...
    for (const r of records) {
        ...
        else if (r.action === DISCOVERY_ACTION)
            discoveryUsed = r.count;
    }
    const total = Object.values(totals).reduce((s, v) => s + v, 0);
    const limit = PLAN_LIMITS[plan].aiCallsPerMonth;
    const norm = (n) => (isFinite(n) ? n : -1); // -1 = unlimited
    return {
