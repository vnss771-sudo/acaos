...
export const EVENT_BASE_WEIGHTS = {
    FUNDING: 95,
    HIRING: 85,
    PROCUREMENT: 90,
    EXPANSION: 75,
...
export function freshnessState(signal, now = Date.now()) {
    const ageDays = Math.max(0, (now - signal.detectedAt.getTime()) / 86400000);
    const rate = SIGNAL_DECAY_RATES[signal.type] ?? 0.01;
...
export function distinctSignalTypes(signals) {
 ...
 */
export function corroborationLevel(signals) {
    const distinctTypes = distinctSignalTypes(signals);
    const level = distinctTypes === 0 ? 'none' : distinctTypes === 1 ? 'single' : distinctTypes === 2 ? 'promising' : 'urgent';
...
function corroborationMultiplier(signals) {
...
function calcIntentScore(signals, signalWeights) {
    if (signals.length === 0)
        return 0;
...
function calcTimingScore(signals) {
    if (signals.length === 0)
        return 10;
    const mostRecentMs = Math.max(...signals.map(s => s.detectedAt.getTime()));
    const ageDays = (Date.now() - mostRecentMs) / 86400000;
    if (ageDays < 3)
        return 100;
    if (ageDays < 7)
        return 90;
...
function calcConfidenceScore(signals) {
    if (signals.length === 0)
        return 10;
...
export function detectBuyingStage(signals, opportunityScore) {
    ...
    if (hasProcurement || (opportunityScore >= 75 && (hasFunding || hasHiring)))
        return 'PURCHASING';
    if ((hasFunding && (hasHiring || hasExpansion)) || opportunityScore >= 65)
        return 'COMPARING';
    if (hasFunding || hasHiring || hasLeadership || opportunityScore >= 45)
        return 'EVALUATING';
    return 'RESEARCHING';
}
// Win probability by buying stage, adjusted by opportunity score
const STAGE_BASE_PROBS = {
    INACTIVE: 0.02,
    RESEARCHING: 0.05,
    EVALUATING: 0.15,
    COMPARING: 0.30,
...
export function calcWinProbability(stage, opportunityScore) {
...
export function generateRuleBasedRecommendation(meta, signals) {
    const sortedByStrength = [...signals].sort((a, b) => EVENT_BASE_WEIGHTS[b.type] - EVENT_BASE_WEIGHTS[a.type]);
    ...
    const ageDays = dominant
        ? (Date.now() - dominant.detectedAt.getTime()) / 86400000
        : 999;
...
export function predictBuyingIntent(signals, currentStage, opportunityScore) {
    ...
    const trajectory = predictedOrder > currentOrder ? 'ACCELERATING' :
        predictedOrder < currentOrder ? 'DECELERATING' :
            'STABLE';
    const recentCount = signals.filter(s => (Date.now() - s.detectedAt.getTime()) / 86400000 < 30).length;
