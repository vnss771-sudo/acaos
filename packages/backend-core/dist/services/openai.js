...
function buildVerticalDesc(icp) {
...
function buildProductDesc(icp) {
    if (icp?.businessType)
        return icp.businessType;
...
export async function generateLeadResearch(input) {
    ...
    return chat(`You are an expert B2B sales intelligence analyst specialising in ${vertical}.
...
- estimatedTeamSize ("1-10" | "10-50" | "50-200" | "200-500" | "500+"): Best estimate from all available signals.`, `Analyse this prospect for B2B cold outreach:
...
export function buildOutreachUserPrompt(input) {
...
export async function generateOutreach(input) {
    ...
    return chat(`You are an elite B2B cold email copywriter. You write for a company selling ${product} to ${vertical}.
...
- followup (string): A 2-sentence follow-up for 4–5 days later if no reply. Acknowledge the first email, offer a slightly different angle or value point. Still ends with a question.`, buildOutreachUserPrompt(input));
}
export async function analyzeReply(replyBody) {
    return chat(`You are a B2B sales intelligence system that classifies cold email replies for field-service software sales teams.
...
- isAutoReply (boolean): true if this appears to be an automated OOO or bounce reply.`, `Classify this B2B cold email reply:
