...
export function detectBounceRecipients(subject, fromAddress, body) {
...
function getRequiredEnv(key) {
    ...
    if (!value)
        throw new ApiError(503, `${key} is not configured`);
    return value;
}
export function isMailConfigured(cfg) {
    return Boolean((cfg?.smtpHost || process.env.SMTP_HOST) && (cfg?.smtpFrom || process.env.SMTP_FROM));
}
export function isMailboxConfigured(cfg) {
    return Boolean((cfg?.imapHost || process.env.IMAP_HOST) &&
        (cfg?.imapUser || process.env.IMAP_USER) &&
        (cfg?.imapPass || process.env.IMAP_PASS));
}
function maybeDecrypt(s) {
    if (!s)
        return undefined;
...
export async function sendMail(to, subject, html, cfg) {
    ...
    if (cfg?.smtpHost)
        await assertPublicMailHost(cfg.smtpHost, 'smtpHost');
...
function extractReplyBody(raw) {
    ...
    for (const line of lines) {
        ...
        if (trimmed.startsWith('>') || /^On .+wrote:/.test(trimmed) || trimmed === '--')
            break;
        cleaned.push(line);
    }
    return cleaned.join('\n').trim();
...
function extractPlainText(source) {
    const raw = source.toString('utf-8', 0, 20000);
...
export async function syncMailboxOnce(cfg, workspaceId) {
    let ImapFlow;
    try {
    ...
    if (cfg?.imapHost)
        await assertPublicMailHost(cfg.imapHost, 'imapHost');
    ...
    try {
        ...
        if (!workspaceId)
            throw new Error('workspaceId required for mailbox sync');
        const existing = await prisma.processedEmail.findMany({
            where: { workspaceId, uid: { gte: windowStart } },
            select: { uid: true, messageId: true }
        });
        const processedRows = existing;
        const seenUids = new Set(processedRows.map((e) => e.uid));
        const seenMsgIds = new Set(processedRows.map((e) => e.messageId).filter(Boolean));
        const toProcess = [];
        let inspected = 0;
        ...
        for await (const msg of client.fetch('*:' + Math.max(1, (client.mailbox?.exists ?? 200) - 199), {
            ...
            if (seenUids.has(uid))
                continue;
            if (messageId && seenMsgIds.has(messageId))
                continue;
            ...
            if (bounceRecipients.length === 0 && replyBody.length < 5)
                continue;
        ...
        if (bounceMsgs.length > 0) {
            ...
            const sentSet = new Set(sentRows.map(r => r.toEmail.toLowerCase()));
        ...
        const typedMatchedLeads = matchedLeads;
        const emailToLead = new Map(typedMatchedLeads.map((l) => [l.email.toLowerCase(), l]));
        let matched = 0;
        let queued = 0;
