...
// account is blocked from mutating (403) but can still read (GET), and verifying
// the email lifts the block.
...
test('an unverified user is blocked from mutating (POST → 403 verification)', async () => {
  const { user, workspace } = await seedUserWithWorkspace() // emailVerified defaults false
  const res = await missions.request('/api/missions', { method: 'POST', headers: auth(user.id), body: createBody(workspace.id) })
  assert.equal(res.status, 403)
  const body = await res.json() as { error?: string }
  assert.match(body.error ?? '', /verification/i, 'must be the email-verification 403, not a tenant/validation 403')
  // Nothing was written.
...
test('onboarding is exempt: an unverified owner may PUT /:id/icp', async () => {
  const { user, workspace } = await seedUserWithWorkspace() // unverified
...
test('a non-onboarding workspace mutation still requires verification (POST /:id/members → 403)', async () => {
  ...
  assert.equal(res.status, 403)
  const body = await res.json() as { error?: string }
  assert.match(body.error ?? '', /verification/i, 'must be the verification 403, proving the exemption is scoped to onboarding')
})
